...
elements = partition_md(filename="outputs/数据仓库工具箱维度建模权威指南（第3版）/2.md")

# 2. 按照标题进行分块
# chunk_by_title 会在遇到新的标题（Title 元素）时启动一个新块
chunks = chunk_by_title(
    elements,
    combine_text_under_n_chars=0,  # 是否合并较小的块，设为0表示严格按标题切分
...
for i, chunk in enumerate(chunks):
    ...
    documents.append(DocumentWrapper(f"{breadcrumb} {" ".join(chunk.text.split())}\n", {
        "filename": "数据仓库工具箱维度建模权威指南（第3版）/2.md", "page_num": i
    }))
...
# 3. 构建工厂类，并使用缓存
generator_llm = llm_factory(provider="google", model="gemini-2.5-pro", client=client, cache=cache)
embeddings = embedding_factory(provider="google", model="gemini-embedding-001", client=client, cache=cache)
...
# 开始构建
apply_transforms(kg, transformers, run_config=custom_config)
...
# %%
df.to_json("outputs/数据仓库工具箱维度建模权威指南（第3版）/2-eval.json")
